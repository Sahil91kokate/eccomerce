...
export default function Product({ subCategory , productData}) {
  ...
  useEffect(() => {
    ...
    if (productData && productData.length > 0) {
      ...
      const uniqueCategories = subCategory.map((subcategory) => subcategory.subCategoryName);
  ...
  useEffect(() => {
    ...
    if (productData && productData.length > 0) {
      const newFilteredProducts = productData.filter((product) =>
        ...
        (selectedCategories.length === 0 || selectedCategories.includes(product.subCategoryName))
      );
...
const ProductsList = () => {
  ...
  <div className="container-fluid mt-5">
      ...
      <div className="col-lg-12 col-md-12 col-sm-12 col-12 pb-1 mt-2">
        ...
        <div className="menu-bar">
          {subCategory?.map((subCategory, index) => (
            <div
              key={index}
              className={`sort-keyword ${selectedCategories.includes(subCategory.subCategoryName) ? 'active' : ''}`}
              onClick={() => handleCategoryClick(subCategory.subCategoryName)}
            >
              {subCategory.subCategoryName}
            </div>
          ))}
            {/* <div className="sort-keyword">All </div> */}
          </div>
        </div> 
    ...
    <h6> Sub  Category</h6> &emsp;   {subCategory?.map((subCategory, index) => (
            <div
              key={index}
              className={`sort-keyword ${selectedCategories.includes(subCategory.subCategoryName) ? 'active' : ''}`}
              onClick={() => handleCategoryClick(subCategory.subCategoryName)}
